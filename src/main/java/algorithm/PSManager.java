    ...
    public void generateChildren(PartialSolution parentPS, PSPriorityQueue queue) {
        ...
        for (Node freeNode: freeNodes) {
            if (parentPS._brokenNodes.size() < 0 || !parentPS._brokenNodes.contains(freeNode)) {
                ...
                if (parentPS._zeroStarts > 1) {
                    ...
                    for (int i = 0; i < _numberOfProcessors; i++) {
                        ...
                        if (parentPS._startingNodes[i] != 0) {
                            ...
                            if (partialSolution._cost == parentPS._cost) {
                                partialBreak = true;
                                brokenNode = freeNode;
                            }
                ...
                if (partialBreak) break;
            }
        }
        if (partialBreak) {
            parentPS._brokenNodes.add(brokenNode);
            queue.add(parentPS);
        } else {
            _cache.add(parentPS);
        }
